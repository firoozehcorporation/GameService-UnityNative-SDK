// <copyright file="Vector4ArraySerializer.cs" company="Firoozeh Technology LTD">
...
{
    internal class Vector4ArraySerializer : ObjectSerializer<Vector4[]>
    {
        ...
        protected override Vector4[] ReadObject(GsReadStream readStream)
        {
            ...
            var data = new Vector4[readData.Length / Base];
            
            for (var i = 0; i < readData.Length; i += Base)
                data[i / Base] = new Vector4(readData[i],readData[i + 1],readData[i + 2],readData[i + 3]);
